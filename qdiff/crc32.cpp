...
#define CALC_CRC32(CRC32, INPUT) CRC32 = ((CRC32) >> 8) ^ CRC32_TABLE[ (INPUT) ^ ((CRC32) & 0x000000FF) ]
...
std::string crc32::calculate(std::string file)
{
  uint32_t  crc_file = 0xFFFFFFFF;
  char      hexstring_crc[10];   //  8 + NULL + 1 extra
  char      input;
  ...
  {
    infile.get(input);
    CALC_CRC32(crc_file, input);
  }

  infile.close();

  crc_file = ~crc_file;
  sprintf(hexstring_crc, "%08lx", static_cast<long unsigned int>(crc_file));

  return std::string(hexstring_crc);
}
